...
public class Scheduler : SingletonBehaviour<Scheduler>
{
    ...
    private List<TimerData> _addingTimerData = new List<TimerData>();
    ...
    public void RemoveTimer(TimerFrameDelegate d)
    {
        ...
        if (_addingTimerData != null)
        {
            for (int j = 0; j < _addingTimerData.Count; j++)
            {
            
                 if (_addingTimerData[j] != null && _addingTimerData[j].TimerHandler == d)
                 {
                      _addingTimerData[j].Destroyed = true;
                 }
    ...
    protected void Update()
    {
        ...
        for (var i = 0; i < _addingTimerData.Count; i++)
        {
            _timerData.Add(_addingTimerData[i]);
        }
        _addingTimerData.Clear();
